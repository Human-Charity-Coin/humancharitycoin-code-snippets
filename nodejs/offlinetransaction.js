 ...
 *
 * Accepts a destination address and a humancharity amount. A raw transaction is
...
console.log('Sending', amount, 'humancharity to', destination_address);
...
async.waterfall([
  function(next) {
    ...
    humancharitycoin.listUnspent(next);
  },
  ...
  function(sum, txs, next) {
    ...
    humancharitycoin.getAccountaddress('change', function(err, change_address) {
      if (err) {
        next(err);
      } else {
  ...
  function(sum, txs, change_address, next) {
    ...
    humancharitycoin.createRawTransaction(inputs, outputs, next);
  },
  function(unsigned_hex, next) {
    console.log('Signing raw transaction...');
    humancharitycoin.signRawTransaction(unsigned_hex, next);
  },
  function(signed, next) {
    if (signed.complete) {
      console.log('\nYour offline transaction is:');
      console.log(signed.hex + '\n');
      console.log('Move this across your air gap and send it to the p2p network using the sendRawTransaction humancharitycoind command.\n');

      humancharitycoin.decodeRawTransaction(signed.hex, next);
    } else {
