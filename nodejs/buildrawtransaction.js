 ...
 * on humancharitycoin testnet and won't be accepted by humancharitycoind running
 * on the main humancharity blockchain (real humancharity addresses start with D).
 */
...
async.waterfall([
    function(next) {
      ...
      var outputs = {};
      /* Leave one humancharity unspent as transaction fee. */
    ...
    function(unsigned_hex, next) {
      ...
      humancharitycoin.signRawTransaction(unsigned_hex, [], [tx.private_key], next);
    },
    ...
    function(signed_hex, next) {
      ...
      humancharitycoin.decodeRawTransaction(signed_hex, next);
    }
